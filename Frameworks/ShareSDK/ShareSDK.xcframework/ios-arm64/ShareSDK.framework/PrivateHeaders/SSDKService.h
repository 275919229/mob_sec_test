 ...
 *  获取配置信息
 */
- (void)getAppConfigWithAppkey:(NSString *)appkey duid:(NSString *)duid result:(void(^)(BOOL statDeviceOn, BOOL statShareOn, BOOL statAuthOn, BOOL backflowOn, BOOL loginplusOn, NSTimeInterval timestamp, NSError *error))result;
 ...
 *  获取设备tags
 */
- (void)getTagsWithAppkey:(NSString *)appkey duid:(NSString *)duid result:(void(^)(NSArray *,NSError *))result;


/// 上报安装量
/// @param appkey appkey
/// @param plat 设备类型: ios/android
/// @param installChannel 渠道名称（iOS填写AppStore即可）
/// @param resultHandler 回调信息
- (void)installationcount:(NSString *)appkey  onResult:(void(^)(NSError *error))resultHandler;
